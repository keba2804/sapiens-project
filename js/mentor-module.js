// ====================================
// MÓDULO DEL MENTOR - VERSIÓN MEJORADA CON NOTIFICACIONES CORREGIDAS
// ====================================
...
async function cargarDashboardMentor(mentorId) {
    ...
    try {
        ...
        setTimeout(() => {
            mostrarNotificacionMentor('✅ Panel de mentor cargado correctamente', 'success');
        }, 500);
...
// ===== ESCUCHAR SOLICITUDES PENDIENTES EN TIEMPO REAL - VERSIÓN CORREGIDA =====
function escucharSolicitudesPendientes(mentorId) {
  console.log("👂 [LISTENER] Iniciando escucha para mentor:", mentorId);
  
  // ⚡ IMPORTANTE: Detener listener anterior si existe
  if (window.mentorListener) {
    console.log("🛑 Deteniendo listener anterior");
    window.mentorListener();
    window.mentorListener = null;
  }
  
  // ⚡ VARIABLE PARA DETECTAR LA PRIMERA CARGA
  ...
  const unsubscribe = db.collection('sessions')
    ...
    .onSnapshot((snapshot) => {
      console.log(`📊 [LISTENER] Snapshot recibido. Total docs: ${snapshot.size}`);
      ...
      snapshot.docChanges().forEach((change) => {
        ...
        console.log(`🔍 [LISTENER] Change type: ${change.type}, Doc ID: ${change.doc.id}`);
        
        if (change.type === 'added') {
          console.log("🆕 [LISTENER] Nueva solicitud:", {
            id: change.doc.id,
          ...
          // ⚡ NO NOTIFICAR EN LA PRIMERA CARGA (solo notificar solicitudes NUEVAS)
          if (!esPrimeraCarga) {
            console.log("🔔 [NOTIFICACIÓN] Mostrando notificación para:", data.student_name);
            
            // 🔔 NOTIFICACIÓN EN TIEMPO REAL
            mostrarNotificacionMentor(`🎉 Nueva solicitud de ${data.student_name}`, 'success');
            ...
            console.log("⏭️ [LISTENER] Solicitud existente (primera carga), no notificar");
          }
        }
        
        if (change.type === 'modified') {
          console.log("✏️ [LISTENER] Solicitud modificada:", change.doc.id);
        }
        
        if (change.type === 'removed') {
          console.log("🗑️ [LISTENER] Solicitud eliminada:", change.doc.id);
        }
      });
      
      // Marcar que ya pasó la primera carga
      if (esPrimeraCarga) {
        console.log("✅ [LISTENER] Primera carga completa. Notificaciones activadas para solicitudes NUEVAS.");
      ...
      console.log(`📬 [LISTENER] Total solicitudes pendientes: ${solicitudes.length}`);
      ...
      console.error("❌ [LISTENER] Error en listener:", error);
      mostrarNotificacionMentor('⚠️ Error de conexión. Reintentando...', 'warning');
      setTimeout(() => {
  ...
  console.log("✅ [LISTENER] Listener registrado y activo");
  return unsubscribe;
}
...
console.log("✅ Módulo Mentor cargado con notificaciones en tiempo real CORREGIDAS");